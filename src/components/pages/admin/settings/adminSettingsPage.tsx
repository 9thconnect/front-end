...
// import { updateAdminProfile } from "@/lib/redux/features/auth/authSlice";
...
const AdminSettingsPage = () => {
  ...
  const admin = useAppSelector((state) => state.auth.data) as Admin | null;
  const token = useAppSelector((state) => state.auth.token) as string;
  const dispatch = useAppDispatch();
    ...
    return (
      <div className="max-w-4xl mx-auto my-8">
        <p className="text-red-500">
          Error: Unable to load admin profile. Please try logging in again.
        </p>
      </div>
    );
  }
